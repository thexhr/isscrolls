...
static void read_names_from_json(int, int);
static void read_action_from_json(int);
static void read_turning_from_json(int);
static void read_places_from_json(int);
static void read_chars_from_json(int, int);

static void
read_chars_from_json(int focus, int generate)
{
	...
	for (i = 0; i < n_oracles; i++) {
		...
		for (j = 0; j < n_entries; j++) {
			...
			if (id == die) {
				...
				log_debug("*** ID %d now larger than the last one %d, we have a gap (die %ld)\n", id, id_temp, die);
				if (generate)
					convert_to_lowercase(temp_name);
				printf("%s", temp_name);
				goto out;
...
static void
read_names_from_json(int person, int generate)
{
	char path[_POSIX_PATH_MAX], temp_name[255];
	json_object *root, *oracles, *temp, *table, *name, *desc, *chance;
	size_t n_oracles, n_entries, i, j;
	long die;
	int what, ret, id, id_temp, max = 100;
...
again:
	die = roll_oracle_die();
	if (person == ORACLE_IS_NAMES) {
		/* For ironander names, we have a range from 0 to 200 so roll twice */
		die += roll_oracle_die();
		max = 200;
	}
	if (die < 0 || die > max)
	...
	log_debug("oracle names die roll is %d\n", die);
...
void
read_moves_from_json(int focus)
{
	char path[_POSIX_PATH_MAX], temp_name[255];
	json_object *root, *oracles, *temp, *table, *name, *desc, *chance;
	size_t n_oracles, n_entries, i, j;
...
static void
read_action_from_json(int focus)
{
	char path[_POSIX_PATH_MAX], temp_name[255];
	json_object *root, *oracles, *temp, *table, *name, *desc, *chance;
	size_t n_oracles, n_entries, i, j;
...
static void
read_turning_from_json(int focus)
{
	char path[_POSIX_PATH_MAX], temp_name[255];
	json_object *root, *oracles, *temp, *table, *name, *desc, *chance;
	size_t n_oracles, n_entries, i, j;
...
static void
read_places_from_json(int focus)
{
	char path[_POSIX_PATH_MAX], temp_name[255];
	json_object *root, *oracles, *temp, *table, *name, *desc, *chance;
	size_t n_oracles, n_entries, i, j;
...
{
	read_names_from_json(ORACLE_IS_NAMES, 1);
	printf(" the ");
	read_chars_from_json(ORACLE_CHAR_ROLE, 1);
	printf(" is a ");
	read_chars_from_json(ORACLE_CHAR_DESC, 1);
	printf(" person whose goal is to ");
	read_chars_from_json(ORACLE_CHAR_GOAL, 1);
	printf(".\n");
}
